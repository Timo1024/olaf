...
export class SudokuBacktracker {
  ...
  public getNeededIterations() : number {
   ...
   */
  usedInRow(row : number, number : number): boolean {
    for (let x = 0; x < 9; x++) {
   ...
   */
  usedInColumn(column : number, number : number): boolean {
    for (let y = 0; y < 9; y++) {
   ...
   */
  usedInSquare(row : number, column : number, number : number): boolean {
    row -= row % 3;
    column -= column % 3;
   ...
   */
  isLocationSafe(row : number, column : number, number : number): boolean {
