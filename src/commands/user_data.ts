...
import { guild_id2 } from "../parameters/server.json";
...
export const UserInfo: Command = {
    ...
    run: async (client: Client, interaction: CommandInteraction) => {
        ...
        const tagged_user : GuildMember | null = interaction.options.get(user_data.options[0].name) as GuildMember | null;
        
        let user : User;
        if(tagged_user){
            user = tagged_user.user
        } else {
        ...
        var erstellt_date = new Date(user.createdAt).toLocaleTimeString();

        const guild : Guild | undefined = client.guilds.cache.get(guild_id2);
